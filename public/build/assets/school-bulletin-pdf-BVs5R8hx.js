...
${i?d(i):``}`}async function S(e){if(e?.verifyUrl)return e.verifyUrl;if(!e?.authenticity?.studentId)return null;try{return(await c(a.url(),{method:`POST`,body:{type:`bulletin`,studentId:e.authenticity.studentId,termId:e.authenticity.termId??null,academicYearId:e.authenticity.academicYearId??null,issuedOn:e.authenticity.issuedOn??new Date().toISOString().slice(0,10)}})).url}catch{return null}}async function C(e,t,n){T(e,`<div class="sheet">${x(t,await S(n))}</div>
<footer class="pied">${u()}</footer>`)}async function w(e,t){let n=[];for(let e of t){let t=await S(e.options);n.push(`<div class="sheet page">
${x(e.fiche,t)}
</div>
<footer class="pied">${u()}</footer>`)}T(e,n.join(`
`))}function T(e,t){let n=`<!doctype html>
<html lang="fr">
<head>
<meta charset="utf-8" />
<title>${h(e)}</title>
...
</html>`,r=new Blob([n],{type:`text/html;charset=utf-8`}),i=URL.createObjectURL(r),a=window.open(i,`_blank`);if(!a){URL.revokeObjectURL(i);return}let o=!1,s=()=>{o||(o=!0,a.focus(),a.print(),window.setTimeout(()=>URL.revokeObjectURL(i),6e4))};a.addEventListener(`load`,s),window.setTimeout(()=>{try{a.document?.readyState===`complete`&&s()}catch{}},250)}function E(){return`
