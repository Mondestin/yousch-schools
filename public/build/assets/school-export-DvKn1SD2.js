...
</html>`)}function m(e,t,n){let r=t.cloneNode(!0);r.querySelectorAll(`.no-print`).forEach(e=>e.remove()),r.querySelectorAll(`img[src]`).forEach(e=>{let t=e,n=t.getAttribute(`src`);n&&!n.startsWith(`data:`)&&!n.startsWith(`blob:`)&&t.setAttribute(`src`,new URL(n,window.location.href).href)});let i=n?.pageSize??(n?.landscape?`A4 landscape`:`A4`),a=n?.pageMargin??`0`,o=!!(r.matches(`.print-bulletin`)||r.querySelector(`.print-bulletin`)||r.getAttribute(`data-print-root`)===`statement`||r.getAttribute(`data-print-root`)===`receipt`||r.getAttribute(`data-print-root`)===`student-document`),s=n?.pageMargin?a:o?`14mm 16mm 16mm`:`0`;d(`<!doctype html>
<html lang="fr">
<head>
<meta charset="utf-8" />
<title>${u(e)}</title>
...
<style>
  ...
  body {
    ...
    ${o?``:`display: flex; align-items: center; justify-content: center; min-height: 100vh;`}
  }
