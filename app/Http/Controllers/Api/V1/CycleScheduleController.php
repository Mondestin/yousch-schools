...
class CycleScheduleController extends Controller
{
    ...
    public function upsert(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'schedules.*.hours.breaks' => ['nullable', 'array'],
            'schedules.*.hours.breaks.*.id' => ['required', 'string', 'max:40'],
            'schedules.*.hours.breaks.*.label' => ['required', 'string', 'max:80'],
            'schedules.*.hours.breaks.*.startsAt' => ['required', 'string', 'max:8'],
            'schedules.*.hours.breaks.*.endsAt' => ['required', 'string', 'max:8'],
